...
module.exports = {
  ...
  async updateListing(listingID, listing) {
    try {
      const newListing = await Listing.findByIdAndUpdate(listingID, listing, {
        new: true,
      });
      return newListing;
    } catch (err) {
  ...
  async getLimitedListings(page, itemPerPage) {
    try {
      const listing = await Listing.find({}, null, {
        sort: { createdAt: -1 },
        skip: page * itemPerPage,
        limit: itemPerPage,
      }).populate("room"); //find().skip(parseInt(nr, 10)).limit(5).populate('room').limit(); optional:skip(pageOptions.page * pageOptions.limit)
      return listing;
    } catch (err) {
      throw err;
    }
  ...
  async getListingsByQuery(query) {
    try {
      const listing = await Listing.find(query, null, {
        sort: { createdAt: -1 },
        limit: 20,
      })
        .populate("room")
